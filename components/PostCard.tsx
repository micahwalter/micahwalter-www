  ...
  return (
    ...
    >
      <article className="h-full transition-transform duration-200 hover:-translate-y-1">
        {/* Thumbnail */}
        {post.coverImage ? (
          <div className={`w-full overflow-hidden rounded-lg bg-gray/10 mb-4 ${featured ? '' : 'aspect-video'}`}>
            <CoverImage
              folderName={post.folderName}
              filename={coverFilename}
              alt={post.title}
              className={`transition-transform duration-300 group-hover:scale-105 ${featured ? '' : 'object-cover'}`}
