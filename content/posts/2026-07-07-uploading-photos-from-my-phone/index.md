...
flowchart TD
    Browser([Phone / browser]) --> UploadPage["/upload form"]

    UploadPage -->|"POST /photos/auth"| AuthFn["photo-upload-auth"]
    AuthFn -->|"HMAC session token"| UploadPage

    UploadPage -->|"POST /photos/upload-url"| InitFn["photo-upload-init"]
    InitFn -->|"presigned PUT URL"| UploadPage

    UploadPage -->|"PUT photo bytes"| S3In[("micahwalter-photo-uploads<br/>uploads/incoming/")]
...
The feature was largely implemented before I picked it up for review. [PR #73](https://github.com/micahwalter/micahwalter-www/pull/73) had been open since June. The code looked reasonable in the diff, but it had never been deployed end-to-end in my AWS account. That was the issue I wanted to close, and it is the same pattern I used for the [newsletter confirmation email post](/posts/newsletter-confirmation-emails) last week.
...
One more issue turned up after I set `NEXT_PUBLIC_PHOTO_API_URL` in GitHub Actions secrets. The deploy workflow passed Fathom and newsletter URLs into the build but not the photo API URL. A one-line fix to `.github/workflows/deploy.yml` and a redeploy baked the value into the static `/upload` page on production.

## Using it

The form lives at [/upload](https://www.micahwalter.com/upload). It is `noindex` and passcode-gated. I pick a JPEG or PNG, optionally set a title and toggle "Feature on homepage," and upload. Within a few minutes I see a new commit on `main`, a GitHub Actions deploy, and the photo in the feed. If I flagged it, the homepage hero updates to the newest featured photo.

JPEG and PNG only for now. The Lambda's Sharp build has no libheif, so HEIC is a follow-up. In practice iOS Safari often transcodes HEIC to JPEG when I pick from the camera roll, which covers the case I care about most.
