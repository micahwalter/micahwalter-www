...
- `scripts/create-post.js` (`blog post:new`) is interactive via stdin prompts and does not work when stdin is piped/non-interactive. To create a post programmatically, write `content/posts/YYYY-MM-DD-<slug>/index.md` directly (frontmatter shape is in `CLAUDE.md`). You do **not** need a local ticket passcode for blog/email posts on a PR: `.github/workflows/allocate-post-ids.yml` calls IAM-authenticated `POST /tickets/allocate` and commits `id` onto the PR branch when a new `content/posts/**/index.md` is added. Photos still get ids from the photo-upload pipeline. The dev server hot-reloads new posts at `/posts/<slug>`.
